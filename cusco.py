...
def process_pep_file(args3):
    ...
    pep_files = [f for f in sorted(os.listdir(f'{input_dir}/fasta')) if f.endswith(('.fasta', '.fas', '.fa', '.fna', '.faa', '.pep', '.seq', '.aln', '.maf', '.nt', '.aa'))]
    temp_files = []
    ...
    if p == True: # Make multi label tree and extend newick file
        ...
        if bootstrap == 0:
            ...
            if reroot:
                ...
                reroot_rotate(reroot, input_rr, output_rr)
                cmd_iq = f'java -Xmx{javaheapspace} -jar ./Astral/astral.5.7.8.jar -i {input_dir}/gene_tree_r.treefiile -o {output_dir}/species_tree.tree 2> {input_dir}/species_nobs_tree.log'
...
def edid(threads, species, insert, eject, restore, input_dir, output_dir):
    ...
    if insert:
        fasta_files = [f for f in os.listdir(f"{input_dir}/fasta") if f.endswith(('.fasta', '.fas', '.fa', '.fna', '.faa', '.pep', '.seq', '.aln', '.maf', '.nt', '.aa'))]
