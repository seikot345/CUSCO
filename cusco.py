...
def wgs2gff(threads, ref_cds, p, outsthreshold, input_dir):
    ...
    wgs_files = [f for f in sorted(os.listdir(f'{input_dir}/species')) if f.endswith(('.fasta', '.fas', '.fa', '.fna', '.seq', '.aln', '.maf', '.nt', '.cds'))] # List of WGS files
    ...
    for wgs_file in wgs_files:
        ...
        if p == True: # when polyploid or WGD sample in ./species, extract copy number form copy_number file
            copy_number_file = None
            for fname in os.listdir(f'{input_dir}'):
                if fname.lower() in ['copy_number.tsv', 'copy_number.csv']:
                    copy_number_file = os.path.join(input_dir, fname)
                    break
            if copy_number_file is None:
                raise FileNotFoundError("There is no copy_number file.")
            ext = os.path.splitext(copy_number_file)[1].lower()
            delimiter = '\t' if ext == '.tsv' else ','
            # read copy_number.csv or copy_number.tsv
            copy_number_df = pd.read_csv(copy_number_file, sep=delimiter, header=None, index_col=0)
            copy_number_df = copy_number_df[1].astype(int)

            if sample_name in copy_number_df: # Annotate with low float only ploidy genome
                if copy_number_df[sample_name] == 1:
                ...
                print(f"{sample_name} not found in copy_number list.")
...
def process_gff2fna(args_na):
    ...
    extensions = ['.fasta', '.fas', '.fa', '.fna', '.seq', '.aln', '.maf', '.nt', '.cds']
    ...
    cmd_mv = f"mv {sample_name}.fna {input_dir}/fasta"
...
def process_gff2faa(args_aa):
    ...
    cmd_mv = f"mv {sample_name}.faa {input_dir}/fasta"
    subprocess.run(cmd_mv, shell=True, check=True)

def gff2fasta(threads, a, input_dir):
    if not os.path.exists(f"{input_dir}/fasta"):
        os.makedirs(f"{input_dir}/fasta")
...
def tsv2scg(p, input_dir): # first step : extract single copy genes from Rtab table
    ori_Rtab = pd.read_csv(f"{input_dir}/geneCNV.Rtab", sep='\t')

    if p == True: # when polyploid or WGD sample in ./species, extract multiple copy genes for the sample
        copy_number_file = None
        for fname in os.listdir(f'{input_dir}'):
            if fname.lower() in ['copy_number.tsv', 'copy_number.csv']:
                copy_number_file = os.path.join(input_dir, fname)
        ...
        # read copy_number and cast to int
        #copy_number_df = pd.read_csv(copy_number_file, sep=delimiter, index_col=0).squeeze().astype(int) # read copy_number.csv or copy_number.tsv
        copy_number_df = pd.read_csv(copy_number_file, sep=delimiter, header=None, index_col=0)
        copy_number_df = copy_number_df[1].astype(int)
        #print(copy_number_df)

        # gene copy numbers only
        data_only = ori_Rtab.iloc[:, 1:].copy()

        # check if all ploidy species are in Rtab
        if not all(species in data_only.columns for species in copy_number_df.index):
            missing = [s for s in copy_number_df.index if s not in data_only.columns]
            raise ValueError(f"Missing species in geneCNV.Rtab: {missing}")

        # align both in same order and cast data_only to int
        data_filtered = data_only[copy_number_df.index].astype(int)
        match_df = data_filtered.eq(copy_number_df.values, axis=1)

        filtered_Rtab = ori_Rtab[match_df.all(axis=1)]
        pre_scg_step1 = filtered_Rtab.iloc[:, 0].to_list()

        #print(f"Filtered {len(pre_scg_step1)} genes based on copy_number file.")
        #print(filtered_Rtab[copy_number_df.index].head())  # debug
...
def process_pep_file(args3):
    ...
    pep_files = [f for f in sorted(os.listdir(f'{input_dir}/fasta')) if f.endswith(('.fasta', '.fas', '.fa', '.fna', '.faa', '.pep', '.seq', '.aln', '.maf', '.nt', '.cds', '.aa'))]
    temp_files = []
    ...
    if ref_gene in scg_orthologs1["reference_gene"].values:
        ...
        for col in scg_orthologs1.columns[1:]:
            ...
            pep_files = glob.glob(os.path.join(f"{input_dir}/fasta", f"{file_name}.*"))
            ...
            if pep_files:
                ...
                print(f"No files found = {pep_files} in {input_dir}/fasta")
...
def load_expected_counts(input_dir):  # Read expected count from copy_number.(tsv|csv)
    candidates = [(f"{input_dir}/copy_number.tsv", '\t'), (f"{input_dir}/copy_number.csv", ',')]
    ...
    raise FileNotFoundError("Neither 'copy_number.tsv' nor 'copy_number.csv' was found in the current directory.")
...
def gff2scg_d(pre_scg, threads, input_dir):
    gff_files = [f for f in sorted(os.listdir(f'{input_dir}/gff')) if f.endswith('.gff')]  # List of GFF files
...
def gff2scg_df_d(strict_scg, threads, input_dir):
...
def process_pep_file_d(args3):
    ...
    for col in scg_orthologs1.columns[1:]:
        ...
        pep_files = glob.glob(os.path.join(f"{input_dir}/fasta", f"{sample_prefix}.*"))
        ...
        if pep_files:
            ...
            print(f"[Warning] No files found for sample '{sample_prefix}' in {input_dir}/fasta")
...
def fasta2scg_d(pre_scg2, scg_orthologs1, threads, input_dir, output_dir): # Create single copy gene FASTA
    if not os.path.exists(f"{output_dir}/single_copy_genes"): # create output directory
        os.makedirs(f"{output_dir}/single_copy_genes")
    ref_genes = scg_orthologs1["reference_gene"].tolist() # reference genes
    ...
    with multiprocessing.Pool(threads) as pool3: # Processing that create singel copy gene fasta with multicore
        args3 = [(ref_gene, scg_orthologs1, input_dir, output_dir) for ref_gene in ref_genes]
        pool3.map(process_pep_file_d, args3)
### up to here ###
...
def process_genetree(args_gt): # Run iqtree
    fasta_file, sub_model, bootstrap, reroot, input_dir, output_dir =  args_gt

    fasta_path = os.path.join(f"{output_dir}/single_copy_genes", fasta_file)
    gene_name = fasta_file.split('.fasta')[0]

    reroot_name = reroot[0] if reroot else None

    if bootstrap == 0:
        if reroot:
            cmd1 = f"iqtree2 -s {fasta_path} -T AUTO --quiet -m {sub_model} --prefix {gene_name} -o {reroot_name}"
        else:
            cmd1 = f"iqtree2 -s {fasta_path} -T AUTO --quiet -m {sub_model} --prefix {gene_name}"
    else:
        if reroot:
            cmd1 = f"iqtree2 -s {fasta_path} -T AUTO --quiet -m {sub_model} --ufboot {bootstrap} --bnni --prefix {gene_name} -o {reroot_name}"
        else:
    ...
    try:
        ...
        success = False  # when return error by iqtree

    if success: # move error files to ./gene_tree/no_infor_gene/
        cmd2 = f"mv {gene_name}.* {input_dir}/gene_tree"
        subprocess.run(cmd2, shell=True, check=True)
    else: # move file to ./gene_tree/ in normal
        error_dir = f"{input_dir}/gene_tree/no_info_gene"
        os.makedirs(error_dir, exist_ok=True)
        for ext in [".log", ".model.gz", ".uniqueseq.phy"]:
            src = f"{gene_name}{ext}"
            dst = os.path.join(error_dir, src)
            if os.path.exists(src):
                shutil.move(src, dst)

def make_gene_tree(threads, sub_model, bootstrap, reroot, input_dir, output_dir): # Make gene tree for species tree
    if not os.path.exists(f"{input_dir}/gene_tree"):
        os.makedirs(f"{input_dir}/gene_tree")
    if not os.path.exists(f"{input_dir}/gene_tree/no_info_gene"):
        os.makedirs(f"{input_dir}/gene_tree/no_info_gene")
    ...
    with multiprocessing.Pool(threads) as poolgt: # Processing that create singel copy gene fasta with multicore
        args_gt = [(fasta_file, sub_model, bootstrap, reroot, input_dir, output_dir) for fasta_file in fasta_files]
        poolgt.map(process_genetree, args_gt)

def safe_reroot_and_rotate(tree: Tree, labels: list):
    targets = [tree.search_nodes(name=label)[0] for label in labels if tree.search_nodes(name=label)] # Get the node corresponding to the label
    if not targets:
        raise ValueError("None of the labels found in tree.")

    if len(targets) == 1: # If there is one target node, use it as the outgroup. If there are multiple target nodes, use the MRCA.
        outgroup_node = targets[0]
    else:
        outgroup_node = tree.get_common_ancestor(targets)

    if outgroup_node != tree: # reroot
        tree.set_outgroup(outgroup_node)

    current = outgroup_node
    for node in outgroup_node.get_ancestors(): # Swap_children logic for outputting to the left end -Adjust so that the target is on the left side from the root to the target node-
        if node.children[0] != current:
            node.swap_children()
        current = node

def reroot_rotate(reroot, input_rr, output_rr):
    with open(input_rr) as fin, open(output_rr, "w") as fout:
        for line_num, line in enumerate(fin, 1):
            ...
            try:
                ...
                print(f"Line {line_num}: failed to parse Newick: {e}")
                continue

            missing = [label for label in reroot if not t.search_nodes(name=label)]
            if missing:
                print(f"Line {line_num}: labels {missing} not found in tree.")
                continue

            try:
                safe_reroot_and_rotate(t, reroot)
            except Exception as e:
                print(f"Line {line_num}: failed to reroot: {e}")
                continue

            fout.write(t.write(format=1) + "\n")

def make_species_tree(threads, bootstrap, reroot, p, pip, input_dir, output_dir): # Make species tree
    if threads == 1:
        javaheapspace = "500M"
    else:
    ...
    if p == True: # Make multi label tree and extend newick file
        ...
        species_count = {}
        with open(f"{input_dir}/copy_number.tsv") as f:
            for line in f:
                if line.strip():
        ...
        df = pd.read_csv(f"{input_dir}/copy_number.tsv", sep='\t', header=None, names=["sample", "ploidy"])
        ...
        if reroot:
            cmd_mv = f'mv {input_dir}/gene_tree.txt {input_dir}/gene_tree_r.txt'
            subprocess.run(cmd_mv, shell=True, check=True)

            input_rr = f'{input_dir}/gene_tree.new'
            output_rr = f'{input_dir}/gene_tree_r.new'
            reroot_rotate(reroot, input_rr, output_rr)

            cmd_polyphest = f'python Polyphest/Polyphest.py --gene_tree_file {input_dir}/gene_tree_r.new --consensus_multiset_file {input_dir}/gene_tree_r.txt --filter_strategy percentile --percentile 75 --use_near_isomorphic True --isomorphic_threshold 0.2'
            subprocess.run(cmd_polyphest, shell=True, check=True)

            cmd_mv = f'mv gene_tree_r-polyphest.txt {input_dir}/'
            subprocess.run(cmd_mv, shell=True, check=True)

            cmd_exsptre = f'head -n 1 {input_dir}/gene_tree_r-polyphest.txt > {output_dir}/species_tree.tree && sed -i "s@multree:\ @@g" {output_dir}/species_tree.tree'
            subprocess.run(cmd_exsptre, shell=True, check=True)

            cmd_exsptere = f'head -n 2 {input_dir}/gene_tree_r-polyphest.txt | tail -n 1 > {output_dir}/species_tree.enewick && sed -i "s@network:\ @@g" {output_dir}/species_tree.enewick'
            ...
            cmd_mv = f'mv gene_tree-polyphest.txt {input_dir}/'
            subprocess.run(cmd_mv, shell=True, check=True)
        ...
        if bootstrap == 0:
            ...
            if reroot:
                input_rr = f'{input_dir}/gene_tree.treefile'
                output_rr = f'{input_dir}/gene_tree_r.treefile'
                reroot_rotate(reroot, input_rr, output_rr)
                cmd_iq = f'java -Xmx{javaheapspace} -jar ./Astral/astral.5.7.8.jar -i {input_dir}/gene_tree_r.treefile -o {output_dir}/species_tree.tree 2> {input_dir}/species_nobs_tree.log'
                subprocess.run(cmd_iq, shell=True, check=True)

            else:
                cmd_iq = f'java -Xmx{javaheapspace} -jar ./Astral/astral.5.7.8.jar -i {input_dir}/gene_tree.treefile -o {output_dir}/species_tree.tree 2> {input_dir}/species_nobs_tree.log'
                subprocess.run(cmd_iq, shell=True, check=True)
            ...
            if reroot:
                input_rr = f'{input_dir}/gene_tree.contree'
                output_rr = f'{input_dir}/gene_tree_r.contree'
                reroot_rotate(reroot, input_rr, output_rr)

                uf_files = [f for f in os.listdir(f"{input_dir}/gene_tree") if f.endswith(".ufboot")]
                uf_files.sort()
                uf_files_r = [f.replace(".ufboot", "_r.ufboot") for f in uf_files]

                with multiprocessing.Pool(threads) as pooluf: # Processing that create9 singel copy gene fasta with multicore
                    args_ufr = [(reroot, os.path.join(input_dir, "gene_tree", input_rr), os.path.join(input_dir, "gene_tree", output_rr)) for input_rr, output_rr in zip(uf_files, uf_files_r)]
                    pooluf.starmap(reroot_rotate, args_ufr)

                cmd_find = f'find {input_dir}/gene_tree/*_r.ufboot > {input_dir}/gene_tree_r.ufboot'
                subprocess.run(cmd_find, shell=True, check=True)

                cmd_iq = f'java -Xmx{javaheapspace} -jar ./Astral/astral.5.7.8.jar -i {input_dir}/gene_tree_r.contree -b {input_dir}/gene_tree_r.ufboot -r {bootstrap} -o {input_dir}/species_bs{bootstrap}_tree.contree 2> {input_dir}/species_bs{bootstrap}_tree.log'
                subprocess.run(cmd_iq, shell=True, check=True)
            else:
...
def rf_calculation(pip, bootstrap, species_tree, input_dir, output_dir):
    ...
    for genetree_file in genetree_files:
        ...
        try:
            rf_values = gene_tree.robinson_foulds(species_tree, unrooted_trees=True) # Get RF distance and maximum RF distance from dictionary
...
def make_candidates(input_dir, output_dir):
    ...
    cmd_sed = f"sed -i 's/:0.00000//g' {input_dir}/candidates_sptree/candidate*tree"
    subprocess.run(cmd_sed, shell=True, check=True) # return to Polyphest output

def rf_calculation_multiple_species(pip, bootstrap, input_dir, output_dir, candidates_dir):
    ...
    for species_file in species_tree_files:
        ...
        for genetree_file in genetree_files:
            ...
            try:
                rf_distance, max_rf = gene_tree.robinson_foulds(species_tree, unrooted_trees=True)[:2]
                normalized_rf = rf_distance / max_rf
    ...
    print("最小RF（DataFrame）:", min_rf.head()) # 追加

    with open(f"{output_dir}/marker.list", "w") as f:
        f.write("gene\tRF\n")
...
def extract_value_from_tsv_d(input_gene, sp, copy_number_value, output_dir):
    with open(f'{output_dir}/single_copy_genes.tsv', newline='', encoding='utf-8') as file:
        reader = csv.DictReader(file, delimiter='\t')
        for row in reader:
            if row[reader.fieldnames[0]] == input_gene:
                sp_name_tsv = [f"{sp}_{i}" for i in range(1, copy_number_value + 1)]
...
def prime(p, target_gene, input_species, input_dir, output_dir):
    ...
    if p == True:
        ...
        for fname in os.listdir(f'{input_dir}/'):
            if fname.lower() in ['copy_number.tsv', 'copy_number.csv']:
                copy_number_file = fname
                break
        if copy_number_file is None:
            raise FileNotFoundError(f"There is no {input_dir}/copy_number file.")
        ext = os.path.splitext(copy_number_file)[1].lower()
        delimiter = '\t' if ext == '.tsv' else ','
    ...
    for num, sp in enumerate(input_species):
        ...
        if p == True:
            copy_number_value = copy_number_df.loc[sp].item()
            geneMP = extract_value_from_tsv_d(target_gene, sp, copy_number_value, output_dir)
        else:
...
'''main commands'''
def prepare(threads, ref_cds, a, p, outsthreshold, input_dir):
    wgs2gff(threads, ref_cds, p, outsthreshold, input_dir)
    paf2tsv(input_dir)
    gff2fasta(threads, a, input_dir)

def curate(threads, p, input_dir, output_dir, pip):
    pre_scg1 = tsv2scg(p, input_dir)
    if p:
        results_dict = gff2scg_d(pre_scg1, threads, input_dir)
        expected_counts = load_expected_counts(input_dir)
        pre_scg2 = extract_strict_scg(results_dict, expected_counts)
        scg_orthologs1 = gff2scg_df_d(pre_scg2, threads, input_dir)
        fasta2scg_d(pre_scg2, scg_orthologs1, threads, input_dir, output_dir)
    else:
...
def phylo(threads, sub_model, bootstrap, reroot, p, input_dir, output_dir, species_tree, pip):
    make_gene_tree(threads, sub_model, bootstrap, reroot, input_dir, output_dir)
    if species_tree == "species_tree.tree":
        make_species_tree(threads, bootstrap, reroot, p, pip, input_dir, output_dir)
    else:
        pass

def marker(p, input_dir, output_dir, pip, bootstrap, species_tree):
    print(f"Calculate nRF using {species_tree}")
    if p == True:
        make_candidates(input_dir, output_dir)
        test_df = rf_calculation_multiple_species(pip, bootstrap, input_dir, output_dir, candidates_dir=f"{input_dir}/candidates_sptree")
        print(test_df)
        #test_df.to_csv("test_df.csv")
    else:
        rf_calculation(pip, bootstrap, species_tree, input_dir, output_dir)

def pipeline(threads, ref_cds, a, p, outsthreshold, sub_model, bootstrap, reroot, input_dir, output_dir, species_tree):
    prepare(threads, ref_cds, a, p, outsthreshold, input_dir)
    pip = True
    curate(threads, p, input_dir, output_dir, pip)
    phylo(threads, sub_model, bootstrap, reroot, p, input_dir, output_dir, species_tree, pip)
    marker(p, input_dir, output_dir, pip, bootstrap, species_tree)
...
def edid(threads, species, insert, eject, restore, input_dir, output_dir):
    if species:
        file_paths = [os.path.join(f"{input_dir}/species", f) for f in os.listdir(f"{input_dir}/species")
              if f.endswith(('.fasta', '.fas', '.fa', '.fna', '.faa', '.pep', '.seq', '.aln', '.maf', '.nt', '.cds', '.aa'))]
        rename_all_wgs(file_paths)

    if insert:
        fasta_files = [f for f in os.listdir(f"{input_dir}/fasta") if f.endswith(('.fasta', '.fas', '.fa', '.fna', '.faa', '.pep', '.seq', '.aln', '.maf', '.nt', '.cds', '.aa'))]
        with multiprocessing.Pool(threads) as pool_in:
            arg_in = [(os.path.splitext(fasta_file)[0], os.path.join(f"{input_dir}/fasta", fasta_file)) for fasta_file in fasta_files]
...
def main():
    ...
    parser_1.add_argument("-a", action="store_true", help="Get amino acid seq of single copy genes (default: nucleotide seq).")
    parser_1.add_argument("-p", action="store_true", help="Providing information of copy_number as input.")
    parser_1.add_argument("-f", "--outsthreshold", type=float, default=0.50, help="Threshold for miniprot --outs when copy_number is detcted (default: 0.50).")
    ...
    parser_2.add_argument("-p", action="store_true", help="Providing information of copy_number as input.")
    ...
    parser_3.add_argument("-rr", "--reroot", type=str, nargs='+', help="The name of species for rerooting.")
    parser_3.add_argument("-p", action="store_true", help="Providing information of copy_number as input.")
    ...
    parser_4.add_argument("-p", action="store_true", help="Providing information of copy_number as input.")
    ...
    parser_5 = subparsers.add_parser("prime", help="Generate multi fasta for primer design.")
    parser_5.add_argument("-p", action="store_true", help="Providing information of copy_number as input.")
    ...
    parser_6.add_argument("-a", action="store_true", help="Get amino acid seq of single copy genes (default: nucleotide seq).")
    parser_6.add_argument("-p", action="store_true", help="Providing information of copy_number as input.")
    parser_6.add_argument("-f", "--outsthreshold", type=float, default=0.50, help="Threshold for miniprot --outs when copy_number is detcted (default: 0.50).")
    ...
    parser_6.add_argument("-rr", "--reroot", type=str, nargs='+', help="The name of species for rerooting.")
...
Command list:\n\
    ...
    prime         Generate multi fasta for primer design.\n\
    ...
    if args.command == "prepare":
        prepare(args.threads, args.ref_cds, args.a, args.p, args.outsthreshold, args.input_dir)
    elif args.command == "curate":
        curate(args.threads, args.p, args.input_dir, args.output_dir, pip=False)
    elif args.command == "phylo":
        phylo(args.threads, args.sub_model, args.bootstrap, args.reroot, args.p, args.input_dir, args.output_dir, species_tree="species_tree.tree", pip=False)
    elif args.command == "marker":
        marker(args.p, args.input_dir, args.output_dir, pip=False, bootstrap=1000, species_tree=args.species_tree)
    elif args.command =="prime":
        prime(args.p, args.target_gene, args.input_species, args.input_dir, args.output_dir)
    elif args.command == "pipeline":
        pipeline(args.threads, args.ref_cds, args.a, args.p, args.outsthreshold, args.sub_model, args.bootstrap, args.reroot, args.input_dir, args.output_dir, species_tree=args.species_tree)
    elif args.command == "edid":
        edid(args.threads, args.species, args.insert, args.eject, args.restore, args.input_dir, args.output_dir)
    else:
        print("Error: {} [command] \n\n    [prepare][curate][phylo][marker][prime][pipeline][edid]\n".format(sys.argv[0]))
        sys.exit(1)
